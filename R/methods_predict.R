...
predict.visitation_model <- function(object,
  ...
  if(trend == "linear"){
    ...
    forecasts <- log(object$constant)+object$beta*time_series_forecasts$seasonality_forecast+object$slope*months_ahead
  }

  ### if difference or only_new = FALSE, combine past observations series with forecasts.
