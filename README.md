...
components of $\Delta$ log$(N_{i,t})$ (where $N_{i,t} > 0$ denote the
actual visitor counts in month $t$ at park $i$). Therefore, by using the
**VisitorCounts** package, one can find the estimated or forecasted
percent change in a park’s visitor counts by only using social media
data for that park. Our model is unique in that, whereas other models
partially or fully utilize on-site visitor counts, ours can achieve
competitive performances on visitation forecasts only with the use of
social media data. In addition, visitation forecasts can be improved by
combining PUD with partial or full on-site visitor counts.
...
#> Warning: package 'VisitorCounts' was built under R version 4.2.3
...
The park_visitation data set will be used to get the Flickr PUD data for
the Yosemite National Park. The flickr_userdays data set will be used as
a proxy for the popularity of Flickr when creating a visitation_model
object.
...
yosemite_nps <- park_visitation[park_visitation$park == "YOSE",]$nps #On site recorded visitation
```

Before we create a Visitation_Model object, we should log-transform both
the flickr_userdays and yosemite_pud for the best results.
...
Now we can create our visitation_model that we will be doing predictions
with:

``` r
yose_visitation_model <- visitation_model(ref_series = log_yosemite_nps, onsite_usage = log_yosemite_pud, omit_trend = TRUE, parameter_estimates = "joint", is_output_logged = FALSE, is_input_logged = TRUE)
...
In this context n_ahead represents the number of months we will be
...
that month in 2022. Alternatively, if n_ahead had been set to 12 I would
only generate forecasts through the year of 2018.
...
$e^{13} = 442,413$, which would be the expected number of visitors based
on our model.
